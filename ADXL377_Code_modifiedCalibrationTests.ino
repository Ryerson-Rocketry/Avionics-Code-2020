...
//----------------------------------------------------------------------------------
  ...
  scaledX = map_float(rawX, 0, 1024, -scale, scale); //1023 for 3.3V, 675(=(3.3/5)*1023) for 5V
  scaledY = map_float(rawY, 0, 1024, -scale, scale);
  scaledZ = map_float(rawZ, 0, 1024, -scale, scale);
...
//float rawXSum=0,Xcorrected = map_float(rawX+(rawX- Xoff)*Xsca,0,1023,-200,200);
    
/* ----------------Summing raw values to find the mean & std deviation==>METHOD 3:--------------------
...
int inputPin = A0;//for accelX
  ...
  // send it to the computer as ASCII digits
    Serial.println("the accelX average with method 3 is:");

  Serial.println(average);
  delay(1);        // delay in between reads for stability

//Serial.print("rawX sum:");
//Serial.println(rawXSum);
*/
//------------------------------------------------------------------------------------------------------------
...
// since AXDL377 is ratiometric the goal is for Voltage = Vs/2=1.65V for each pin, giving a rawAccel value ~=512
  ...
  Serial.print(rawX);
    Serial.print(",      ");
  Serial.print("scaled accelX(g):");
  Serial.print(scaledX);
    Serial.print(",     ");

  Serial.print("adjusted accelX(g):");
